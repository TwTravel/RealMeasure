...
void remove_background(rs2::video_frame& other_frame, const rs2::depth_frame& depth_frame, float depth_scale, float clipping_dist)
{
	...
	if(CPic.Width != width)
    {
		...
		//CPic.FormatF(width, height);
		gColorImg.FormatF( width , height);
		array_buff.resize( width * height);
    }
	
	//reading depth image from camera
	for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < width; x++, ++depth_pixel_index)
        {
	          ...
	          float dist = pixels_distance;   
	          array_buff[depth_pixel_index] = p_depth_frame[depth_pixel_index];
        }
    }
	...
	//#############################################################
	sprintf(text_buff,"framedat/depth%03i.dat", global_count);
	FILE*file = fopen(text_buff,"wb+");
	fwrite(&array_buff[0], sizeof(double), 1280 * 720, file);
	fclose(file);
	   
	sprintf(text_buff,"framedat/color%03i.bmp", global_count);
	gColorImg.Save(text_buff);
