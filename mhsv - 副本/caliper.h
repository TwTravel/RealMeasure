#ifndef Rotate_Caliper_Head
#define Rotate_Caliper_Head
...
struct RotRectangle// MinAreaState
{
	float x, y, angle;
    //int bottom;
   // int left;
...
RotRectangle rotatingCalipers( const RPoint* points, int n, int mode )
{
	RPoint res_out[3];
	float* out = (float*)res_out;
    ...
    RPoint* vect = (RPoint*)(inv_vect_length + n);
    ...
    for( i = 0; i < n; i++ )
    {
        ...
        RPoint pt = points[(i+1) & (i+1 < n ? -1 : 0)];
	...
	RotRectangle box;
	box.x = res_out[0].x + (res_out[1].x + res_out[2].x)*0.5f;
	box.y = res_out[0].y + (res_out[1].y + res_out[2].y)*0.5f;
	box.width = (float)sqrt((double)res_out[1].x*res_out[1].x + (double)res_out[1].y*res_out[1].y);
	box.height = (float)sqrt((double)res_out[2].x*res_out[2].x + (double)res_out[2].y*res_out[2].y);
	box.angle = (float)atan2((double)res_out[1].y, (double)res_out[1].x);
	return box;
}
